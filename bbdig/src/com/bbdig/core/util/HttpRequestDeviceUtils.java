...
public class HttpRequestDeviceUtils {
  ...
  //判断手机客户端访问  
  	/*	Enumeration<?> e_headerName = request.getHeaderNames();
  		StringBuffer browser_userAgent_str;
  		browser_userAgent_str = new StringBuffer();

  		while (e_headerName.hasMoreElements()) {
  			browser_userAgent_str.append(request.getHeader(
  					e_headerName.nextElement().toString()));
  		}
  		String cur_userAgent = browser_userAgent_str.toString();
  		//System.out.println("++++++"+cur_userAgent);
  		
  		//String[] keywords = { "android", "iphone", "ipod", "windows phone", "mqqbrowser" ,"mobile" ,"wap"};
  		String[] keywords = { "windows 98","windows me","windows 2000","windows xp","windows nt","ubuntu"};
  		
  		boolean equip_ismobile = false;
  		
  		for (String key : keywords) {
  			if(cur_userAgent.toLowerCase().indexOf(key)!=-1){
  				equip_ismobile = true;
  				break;
  			}
  		}
  		if(!equip_ismobile)
  			request.getSession().setAttribute("USER_EQUIP","mobile");
  		else
  			request.getSession().setAttribute("USER_EQUIP","PC");
  		
  		if(request.getSession().getAttribute("USER_EQUIP").toString().equals("mobile")){//手机
